...
def get_auth_token():
    ...
    config_text = open(GameUserSettingsPath, encoding="utf-16le").read().replace("\ufeff","")
    GameUserSettings = configparser.ConfigParser(strict=False)
    GameUserSettings.read_string(config_text)

    old_refresh_token = ''
    if 'RememberMe' in GameUserSettings.sections():
        if GameUserSettings['RememberMe']['Enable'] == 'True':
            data = GameUserSettings['RememberMe']['Data']
            old_data = data
    ...
    if old_refresh_token:
        ...
        config_text = open(GameUserSettingsPath, encoding="utf-16le").read()
        config_text = config_text.replace(old_data, new_data)
        open(GameUserSettingsPath, 'w+',  encoding="utf-16le").write(config_text)
